...
export class CreateExperienceComponent implements OnInit {
    ...
    switch (step) {
      case 1: 
        if (this.tempSlots().length === 0 && this.existingSlots().length === 0) {
          ...
          this.notificationService.showError('Please add at least one available date/time slot.');
          return false;
        }
        return true;
  ...
  checkControls(controls: string[]): boolean {
    ...
    if (!isValid) {
      ...
      this.notificationService.showError('Please fill in all required fields marked with *');
    }
    return isValid;
  }
  ...
  addSlot(): void {
    if (this.tempSlots().length >= 6) {
      this.notificationService.showError('You can add up to 6 slots only.'); // ✅
      return;
    }

    if (!this.newSlotDate || !this.newSlotTime) {
      this.notificationService.showError('Please select date and start time.'); // ✅
      return;
    }
    ...
    if (selectedDate < now) {
      this.notificationService.showError('⚠️ Cannot select a past time.');
      return;
    }
    ...
    if (existsInTemp || existsInDB) {
      this.notificationService.showError('⚠️ This date and time is already selected!');
      return;
    }
  ...
  async deleteExistingSlot(slotId: number): Promise<void> {
    const confirmed = await this.notificationService.confirmAction('Delete Schedule?', 'Are you sure you want to delete this slot?');
    if (!confirmed) return;

    this.experienceService.deleteAvailability(slotId).subscribe({
      next: () => {
        this.existingSlots.update(slots => slots.filter(s => s.id !== slotId));
        this.notificationService.showToast('success', 'Slot deleted');
      }
    });
  }
    ...
    if (this.isEditMode && this.experienceId) {
      ...
      this.experienceService.uploadImage(this.experienceId, file).subscribe({
        next: (response) => {
          ...
          this.notificationService.showToast('success', 'Image uploaded'); // ✅
        },
        error: () => {
  ...
  async deleteImage(imageId: number): Promise<void> {
    const confirmed = await this.notificationService.confirmAction('Delete Image?', 'This action cannot be undone.');
    if (!confirmed) return;

    this.experienceService.deleteImage(imageId).subscribe({
      next: () => {
        this.loadExperienceDetails(this.experienceId!);
        this.notificationService.showToast('success', 'Image deleted');
      }
    });
  }
  ...
  async onSubmit(): Promise<void> {
    if (this.experienceForm.invalid) {
      ...
      this.notificationService.showError('Please check previous steps for missing information.'); 
      return;
    }
    ...
    try {
      ...
      this.notificationService.showSuccess('Success', 'Experience saved successfully!');
      this.router.navigate(['/host/experiences']);

    } catch (err: any) {
      console.error('Save Error:', err);
 
      this.notificationService.showError(err.error?.message || 'Failed to save');
    } finally {
